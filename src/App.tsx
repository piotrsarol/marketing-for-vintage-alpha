...
type SettingsData = { product: { name: string; url: string; description: string; country: string; language: string; audience: string[]; callToAction: string; searchQuery?: string }; ai: { provider: string; model: string; lastRequest: 'openai' | 'mock' | 'unknown'; lastError: 'not_configured' | 'http_error' | 'empty_response' | 'invalid_json' | 'network_or_timeout' | null }; publishing: { provider: string; configured: boolean }; storage: string; automation: { dailyWorkflow: boolean; workflowFile: string }; updatedAt?: string }
const trends: Trend[] = []
