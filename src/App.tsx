...
function Overview({ data, onExplore, onQueue }: { data: DashboardData | null; onExplore: () => void; onQueue: () => void }) {
  ...
  return <section className="page-section"><PageIntro eyebrow="CAMPAIGN CONTROL CENTER" title="Marketing engine" description="Discover signals, generate campaigns, queue distribution, and measure demand from one authenticated workspace." action={<button className="primary-button" onClick={onExplore}>Open trend radar <span>↗</span></button>} /><div className="metrics"><Metric label="Signals discovered" value={String(data?.trends.length || 0)} change="live" note="stored signals" icon="✦" /><Metric label="Campaigns generated" value={String(data?.campaigns.length || 0)} change="live" note="saved campaigns" icon="▣" /><Metric label="Queued assets" value={String(data?.queue.length || 0)} change="live" note="awaiting publisher" icon="◷" /><Metric label="Landing conversion" value={`${conversion}%`} change="live" note={`${data?.funnel.signups || 0} signups`} icon="↗" /></div><div className="dashboard-grid"><div className="panel trend-panel"><div className="panel-heading"><div><span className="eyebrow">LATEST SIGNALS</span><h2>Discovered and scored</h2></div><button className="text-button" onClick={onExplore}>View all ↗</button></div>{latestTrends.map((trend, index) => <div className="trend-row" key={trend.id}><div className="rank">{String(index + 1).padStart(2, '0')}</div><div className="trend-info"><b>{trend.topic}</b><span>{trend.category} · {trend.source}</span></div><div className="trend-score"><strong>{trend.score}</strong><small>score</small></div><span className="trend-delta">{trend.status}</span></div>)}{!latestTrends.length && <EmptyState title="No signals yet" description="Run the campaign engine to discover and score the first trend." />}</div><div className="panel activity-panel"><div className="panel-heading"><div><span className="eyebrow">FUNNEL</span><h2>Demand captured</h2></div></div><div className="chart-label"><strong>{data?.funnel.signups || 0}</strong><span>signups</span></div><p className="empty-copy">{data?.funnel.pageViews || 0} tracked landing visits · {conversion}% conversion</p></div></div><div className="panel queue-panel"><div className="panel-heading"><div><span className="eyebrow">DISTRIBUTION</span><h2>Publishing queue</h2></div><button className="text-button" onClick={onQueue}>View queue ↗</button></div>{queued.map((item) => <div className="queue-item compact" key={item.id}><div className="social-icon dark">{item.platform.slice(0, 2)}</div><div className="queue-copy"><b>{item.platform} asset</b><span>{new Date(item.scheduledFor).toLocaleString()}</span></div><span className={`status ${item.status}`}>{item.status}</span></div>)}{!queued.length && <EmptyState title="Queue is empty" description="Generated content will appear here once a publisher is configured." />}</div></section>
}
